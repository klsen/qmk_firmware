...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [_base] = LAYOUT_65_split_space(
        KC_GESC, KC_1,    KC_2,    KC_3,    KC_4,    KC_5,    KC_6,    KC_7,    KC_8,    KC_9,    KC_0,    KC_MINS, KC_EQL,  BSPC2,   KC_MPLY,
    ...
    [_fn] = LAYOUT_65_split_space(
        ...
        _______, XXXXXXX, XXXXXXX, XXXXXXX, RESET,   XXXXXXX, XXXXXXX, XXXXXXX, IAN,     XXXXXXX, XXXXXXX, AG_SWAP, AG_NORM, _______, _______,
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    static bool ian_mode = false;

    switch(keycode) {
        case BSPC2:
            if (record->event.pressed) register_code(KC_BSPC);
            else unregister_code(KC_BSPC);
            return true;
        case IAN:
            if (record->event.pressed) ian_mode = !ian_mode;
            return true;
        default:
            ...
            if (swap_space && (keycode == KC_SPC || keycode == KC_BSPC || keycode == KC_DEL)) {
                if (record->event.pressed) {
                    if (keycode == KC_SPC) {
                        if (ian_mode) register_code(KC_LCTL);
                    ...
                    if (ian_mode) unregister_code(KC_LCTL);
